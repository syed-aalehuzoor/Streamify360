        ...
        for quality in qualities:
            ...
            if logo_file and os.path.isfile(logo_file) and subtitle_file and os.path.isfile(subtitle_file):
                command = [
                    ...
                    "-i", logo_file,
                    "-filter_complex", f"[1:v][0:v]scale2ref=h=ow/mdar:w=iw[logo][video];[video][logo]overlay=format=auto,ass='{subtitle_file}',scale={video_setting}",
                ...
                command = [
                    "ffmpeg", "-y", "-copyts", "-i", chunk_path,
                    "-i", logo_file,
                    "-filter_complex", f"[1:v][0:v]scale2ref=h=ow/mdar:w=iw[logo][video];[video][logo]overlay=format=auto,scale={video_setting}",
                    "-map", "0", "-b:v", bitrate, "-c:a", "aac", "-b:a", "128k", "-crf", "27", "-preset", "veryfast",
                    "-hls_time", str(chunk_duration), "-hls_list_size", "0",
                    "-hls_flags", "single_file",
                    "-hls_segment_filename", chunk_output_path,
                ...
                #ffmpeg -i input_video.mp4 -i logo.png -filter_complex "[1:v][0:v]scale2ref=h=ow/mdar:w=iw[logo][video];[video][logo]overlay=format=auto,ass=subtitles.ass" -c:v libx264 -preset fast -c:a aac -b:a 128k -movflags +faststart output.mp4
